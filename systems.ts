...
export class AugmentedMatrix3 {
    ...
    getSolutionSetArraysByBackSubstitution(): { text_solution: Array<string>; coeff_solution: Array<Record<string, Frac>> } {
        ...
        if (secondRow.hasOneUnknown()) { // solve for unknown
            ...
            let newFirstRowArray: Array<number> = [];

            firstRowFracArray.forEach((fracElement) => { // scale by denominator so integer first row
...
export function generateUniqueSolutionExercise(max: number = 10) {

    let M1 = getRandomAugmentedMatrix3(max, true);
    const answer = M1.getFracSolution();

    const generated_exercise: { M1: AugmentedMatrix3; answer: Array<Frac> } = {
        M1: M1,
        answer: answer
    }
...
const testAugmentedMatrix2 = new AugmentedMatrix3(
    1, -1, 1, 3,
    0, 0, -1, 6,
    0, 0, 0, 0
);
